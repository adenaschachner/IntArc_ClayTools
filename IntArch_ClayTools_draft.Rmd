...
```{r full model}
ModelFull = function(Box, HandleSimilarity, RodSimilarity, HandleChoices, RodChoices, priorCopy, Response){
...
ModelISC = function(Box, HandleSimilarity, RodSimilarity, HandleChoices, RodChoices, priorCopy, Response){
...
CopyLlhFull = function(priorCopy){
  tempData = DatCopy2 %>%
    rowwise() %>%
    mutate(pred = ModelFull(Box, HandleSimilarity, RodSimilarity, HandleChoices, RodChoices, priorCopy, Result))
...
LwrsFull = c(0.000000001)
UprsFull = c(0.999999999)
InitsFull = list(priorCopy = mean(DatCopy2$Result == "Someone copied."))
...
CopyLlhISC = function(priorCopy){
  tempData = DatCopy2 %>%
    rowwise() %>%
    mutate(pred = ModelISC(Box, HandleSimilarity, RodSimilarity, HandleChoices, RodChoices, priorCopy, Result))
  -sum(log(tempData$pred))
}
```
...
InitsNChoice = list(priorCopy = mean(DatCopy2$Result == "Someone copied."),
             nChoices = round(mean(c(DatCopy2$HandleChoices, DatCopy2$RodChoices))))
